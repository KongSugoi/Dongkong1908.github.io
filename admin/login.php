...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit1']) && !$blocked) {
    ...
    if ($username === '' || $password === '') {
        ...
        $stmt = $link->prepare("SELECT id, password, status FROM admin_registration WHERE username = ?");
        if ($stmt) {
            ...
            if ($stmt->num_rows === 1) {
                ...
                if ($status !== 'yes') {
                    ...
                    $error = "Không hợp lệ: Sai tên đăng nhập hoặc mật khẩu.";
                   ...
                   if (password_verify($password, $db_password_hash)) {
                    ...
                    if (password_needs_rehash($db_password_hash, PASSWORD_DEFAULT)) {
                        ...
                        $rehashStmt = $link->prepare("UPDATE admin_registration SET password = ? WHERE id = ?");
                        if ($rehashStmt) {
...
<div class="login_wrapper">
    <section class="login_content">
        <form method="post" autocomplete="off" novalidate>
            ...
            <div>
                <input type="text" name="username" class="form-control" placeholder="Tên đăng nhập" required maxlength="100" />
            </div>
            <div>
                <input type="password" name="password" class="form-control" placeholder="Mật khẩu" required maxlength="256" />
            </div>
            <div>
