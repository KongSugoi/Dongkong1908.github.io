...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit1']) && !$blocked) {
    ...
    if ($username === '' || $password === '') {
        ...
        $stmt = $link->prepare("SELECT id, password, status FROM user_registration WHERE username = ?");
        if ($stmt) {
                ...
                if ($status !== 'yes') {
                   ...
                   if (password_verify($password, $db_password_hash)) {
                    ...
                    if (password_needs_rehash($db_password_hash, PASSWORD_DEFAULT)) {
                        ...
                        $rehashStmt = $link->prepare("UPDATE user_registration SET password = ? WHERE id = ?");
                        if ($rehashStmt) {
...
<div class="login_wrapper">
    <section class="login_content">
        <form method="post" autocomplete="off" novalidate>
            ...
            <div class="separator">
                <p class="change_link">Người dùng mới?
                    <a href="registration.php"> Tạo tài khoản </a>
                </p>
